...
namespace CzadRoom.Controllers {
    ...
    public class ChatController : Controller {
        ...
        public async Task<IActionResult> Index() {
            var rooms = (await _roomService.GetAll()).Select(x => _mapper.Map<RoomViewModel>(x));
            return View(rooms);
    }
    ...
    public async Task<IActionResult> Room(string roomId) {
        ...
        await _roomService.UserConnected(roomId, userID);
        var roomDB = await _roomService.GetRoom(roomId);
        var room = _mapper.Map<RoomViewModel>(roomDB);
        return View(room);
    }
